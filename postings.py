...
class Posting:
    ...
    def __init__(self, ID, frequency, url, total_terms, title):
        self.ID = ID
    ...
    def to_dictionary(self): #convert a Posting into a dictionary of its attributes
        return {'ID': self.ID, 'frequency': self.frequency, 'url': self.url, 'total_terms': self.total_terms, 'title': self.title}
